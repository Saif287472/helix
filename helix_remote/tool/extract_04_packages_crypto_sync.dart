...
// Extracts: crypto + sync packages (heaviest protocol logic)
// Output:   helix/docs/codebase/remote_4_packages_crypto_sync.txt
...
void main() async {
  ...
  for (final pkg in ['helix_remote_crypto', 'helix_remote_sync']) {
  ...
  await _write(
    docsDir: docsDir,
    name: 'remote_4_packages_crypto_sync',
    title: 'HELIX-REMOTE  ·  PACKAGES: CRYPTO + SYNC',
    subtitle:
        'helix_remote_crypto (X3DH · double-ratchet · backup · group · prekey · attachment) · helix_remote_sync (sync engine)',
    repoRoot: repoRoot,
    files: files,
  );
}
