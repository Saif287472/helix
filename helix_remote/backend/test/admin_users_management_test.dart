...
void main() {
  ...
  test('non-admin cannot list, suspend, unsuspend, or delete users', () async {
    await registerUser(accountId: 'protected_user', phoneHash: 'protected_ph');

    const badToken = 'not_a_real_admin_token';
    expect((await getJson('/api/v1/ops/users', token: badToken)).statusCode,
        equals(403));
    expect(
      (await postJson(
  ...
  group('Suspend / unsuspend (temporary revoke)', () {
    test(
      'suspending a user blocks login and rejects an already-issued access '
      'token, and unsuspending restores both',
      () async {
      ...
      () async {
        ...
        expect(
          users
              ...
              .where((u) => u['account_id'] == 'delete_user'),
          isEmpty,
        );
      },
    );
  ...
  group('Invite cancellation', () {
    test(
      'cancels a pending invite so it can no longer be redeemed',
      () async {
        ...
        final invites =
            (jsonDecode(list.body) as Map<String, dynamic>)['invites']
                as List;
