...
void main() {
  ...
  // RP5-001 — Upload requires valid authorization
  test(
    'uploadAttachment throws StateError when authorization token is invalid',
    () async {
      final service = RemoteAttachmentService(
        baseUrl: 'http://127.0.0.1:$port',
      ...
      final plaintextFile = File(p.join(clientTempDir.path, 'no_auth.txt'));
      await plaintextFile.writeAsBytes(List.generate(50, (i) => i));
      // prepareAttachment is local-only — succeeds regardless of token value
      final prepResult = await service.prepareAttachment(plaintextFile);
      final attachmentId = prepResult['attachment_id'] as String;
      final cipherPath = prepResult['ciphertext_path'] as String;

      // Upload hits the auth middleware first; invalid JWT → 403 → StateError
      await expectLater(
  ...
  // RP5-003 — Download requires valid authorization
  test(
    'downloadAttachment throws StateError when authorization token is invalid',
    () async {
      // Upload with valid token so the file exists on the server
      final validService = RemoteAttachmentService(
        baseUrl: 'http://127.0.0.1:$port',
        authToken: token,
        db: db,
        tempDir: clientTempDir,
      ...
      final plaintextFile = File(p.join(clientTempDir.path, 'dl_auth.txt'));
      await plaintextFile.writeAsBytes(List.generate(50, (i) => i));
      final prepResult = await validService.prepareAttachment(plaintextFile);
      final attachmentId = prepResult['attachment_id'] as String;
      final cipherPath = prepResult['ciphertext_path'] as String;
      ...
      await validService.uploadAttachment(
        attachmentId: attachmentId,
        ciphertextPath: cipherPath,
      );

      // Download with invalid token is rejected (401/403 → StateError)
      final invalidService = RemoteAttachmentService(
        baseUrl: 'http://127.0.0.1:$port',
        authToken: 'invalid_jwt_token',
        db: db,
        tempDir: clientTempDir,
        wrappingKey: wrappingKey,
      );
      final downloadPath = p.join(clientTempDir.path, 'dl_auth_denied.enc');
      await expectLater(
        () => invalidService.downloadAttachment(
          attachmentId: attachmentId,
          savePath: downloadPath,
        ),
        throwsStateError,
  ...
  // RP5-002 — registerReference happy path
  test('registerReference succeeds for an authorized uploader', () async {
    final service = RemoteAttachmentService(
      baseUrl: 'http://127.0.0.1:$port',
      authToken: token,
      db: db,
    ...
    final plaintextFile = File(p.join(clientTempDir.path, 'ref_test.txt'));
    await plaintextFile.writeAsBytes(List.generate(100, (i) => i));
    final prepResult = await service.prepareAttachment(plaintextFile);
    final attachmentId = prepResult['attachment_id'] as String;
    final cipherPath = prepResult['ciphertext_path'] as String;
  ...
  // RP5-002 — registerReference denied without valid auth
  test(
    'registerReference throws StateError when authorization is invalid',
    () async {
      final service = RemoteAttachmentService(
        baseUrl: 'http://127.0.0.1:$port',
        authToken: 'bad_jwt_token',
        db: db,
        tempDir: clientTempDir,
        wrappingKey: wrappingKey,
      );

      // Auth middleware rejects the call before the handler runs → StateError
      await expectLater(
  ...
  // RP5-005 — Missing local ciphertext file
  test(
    'uploadAttachment throws StateError when local ciphertext file does not exist',
    () async {
      final service = RemoteAttachmentService(
        baseUrl: 'http://127.0.0.1:$port',
        authToken: token,
      ...
      await expectLater(
        () => service.uploadAttachment(
          ...
          ciphertextPath: p.join(clientTempDir.path, 'does_not_exist.enc'),
        ),
        throwsStateError,
      );
    },
