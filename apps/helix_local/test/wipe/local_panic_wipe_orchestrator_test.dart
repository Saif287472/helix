...
void main() {
  ...
  group('WipeResult.succeeded semantics (DEFECT-8 fix)', () {
    ...
    test('DEFECT-8 fix: partialFailure phase with empty errors → succeeded == false', () {
      ...
      expect(result.succeeded, isFalse,
          reason: 'partialFailure phase must be false even with empty errors list');
    });
  ...
  group('Orchestrator happy path', () {
    test('All steps succeed → WipePhase.complete and succeeded == true', () async {
  ...
  group('Orchestrator failure paths', () {
    test('One step throws → WipePhase.partialFailure; remaining steps still run', () async {
      final executed = <String>[];
      final orch = LocalPanicWipeOrchestrator.withSteps([
        ('step_a', () async => executed.add('a')),
        ('step_b', () async { throw Exception('DB deletion failed'); }),
        ('step_c', () async => executed.add('c')),
      ]);
    ...
    test('Multiple step failures → all errors recorded', () async {
      final orch = LocalPanicWipeOrchestrator.withSteps([
        ('step_a', () async { throw Exception('fail a'); }),
        ('step_b', () async { throw Exception('fail b'); }),
      ]);
  ...
  group('Pending-wipe marker', () {
    test('onMarkWipePending is called before any steps', () async {
      ...
      expect(order.first, equals('markPending'),
    ...
    test('onClearWipePending is called after successful wipe', () async {
      ...
      expect(result.succeeded, isTrue);
      expect(order.last, equals('clearPending'),
          reason: 'clearPending must be called after all steps succeed');
    });

    test('onClearWipePending is NOT called after partial failure (preserves retry marker)', () async {
      final cleared = <bool>[];
      final orch = LocalPanicWipeOrchestrator.withStepsAndRecovery(
        [('step', () async { throw Exception('fail'); })],
      ...
      expect(cleared, isEmpty,
          reason: 'clearPending must NOT be called after failure — '
  ...
  group('Idempotency', () {
    test('execute() called twice returns same result, steps run only once', () async {
      int runCount = 0;
      final orch = LocalPanicWipeOrchestrator.withSteps([
        ('step', () async => runCount++),
      ]);
      ...
      expect(runCount, 1, reason: 'Steps must not run again on second execute()');
    });

    test('Failed wipe is also idempotent — second call returns same partialFailure', () async {
      int runCount = 0;
      final orch = LocalPanicWipeOrchestrator.withSteps([
        ('step', () async {
          runCount++;
