...
`.\scripts\verify.ps1` runs `flutter pub outdated` as an advisory in CI, or
when `HELIX_DEPENDENCY_ADVISORY=1` is set locally. Routine local verification
also passes `--no-pub` to Flutter commands unless that variable is set, so
dependency availability checks do not run during every local test pass.
Dependency updates still require human review for maintenance, license,
