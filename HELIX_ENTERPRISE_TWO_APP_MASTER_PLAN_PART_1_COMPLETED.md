# Helix Enterprise Two-App Master Plan - Part 1: Completed Work

**Generated from:** HELIX_ENTERPRISE_TWO_APP_MASTER_PLAN.md on 2026-06-19
**Purpose:** Historical reference for the completed Helix two-app foundation through Phase 11.

Future implementation agents should normally read **Part 2 - Remaining Work** instead of this file. Use this file only when auditing historical decisions, completed deliverables, rollback context, or externally blocked foundation items.

## Current Foundation Status

Phases 0-11 are treated as complete for forward development, except for the externally blocked items listed below. These blockers must remain documented and must constrain security claims, but future application work should continue without waiting for them.

## Externally Blocked Items To Preserve
...
## Completed Closure Repairs After The Original Monolith

- Sync inbound scope now uses a global account/device stream cursor; message events carry their own authoritative `conversation_id`.
- Remote sync dispatch now has typed handlers for message deletion/tombstones, read receipts, delivery receipts, membership changes, conversation creation, message creation, and sync markers.
- Unknown Remote sync events are skipped with redacted diagnostics and do not crash the client.
- Remote client storage includes `processed_event_ids`; duplicate event IDs are suppressed and sequence regressions fail closed.
- Scenario B has executable current symmetric-chain coverage; full successful 3->1->2 skipped-key delivery remains blocked with the DH/skipped-key ratchet item.
- Scenario G verifies Local panic wipe leaves Remote DB/key material and Remote endpoints untouched.
- Scenario H verifies Remote startup fails closed when the DB key is absent and does not create a plaintext fallback DB.
- CI workflow and export tooling were added/updated for the closure evidence path.

## Historical Source Material

The sections below preserve the completed foundation decisions, phase deliverables, and work log from the original monolithic master plan.

## Original Plan Sections 0-5: Foundation Decisions And Execution Protocol
...
## Original Phase 0-11 Deliverables
...
- [x] **P6-001:** Inventory every current call to `HelixDatabase`.
- [x] **P6-002:** Identify all persisted message, thread, draft, archive, pin, one-way, and peer-cache behavior.
- [x] **P6-003:** Replace Local conversation persistence with `LocalEphemeralConversationRepository`.
- [x] **P6-004:** Keep chat threads/messages in RAM only.
- [x] **P6-005:** Keep drafts in RAM only.
- [x] **P6-006:** Keep one-way inbox in RAM only and enforce TTL.
- [x] **P6-007:** Keep group/lobby message history in RAM only.
- [x] **P6-008:** Do not persist call history.
- [x] **P6-009:** Decide whether favorites/trusted nicknames persist; document separately from conversations.
- [x] **P6-010:** Split `HelixDatabase` into explicitly named Local metadata storage or retire it from content paths.
...
- [x] **P6-024:** Create a Local-only `LocalPanicWipeOrchestrator`.
- [x] **P6-025:** Define an ordered wipe state machine.
- [x] **P6-026:** Block new network/session activity once wipe starts.
...
- [x] **P6-050:** Seed Local and Remote test storage.
- [x] **P6-051:** Run Local panic wipe.
...
- [x] **P10-003:** Refresh-token rotation.
...
- [x] **P10-027:** Staging deployment.
- [x] **P10-028:** Secret management.
- [x] **P10-029:** TLS configuration.
- [x] **P10-030:** Metrics, logs, and traces.
- [x] **P10-031:** Backup and restore test.
- [x] **P10-032:** Dependency and container scanning.
- [x] **P10-033:** API load test baseline.
- [x] **P10-034:** Incident response runbook.
...
- [x] Staging is isolated from production.
...
## Historical Work Log Through Phase 9-11 Closure
...
### Phase 6 — Make Helix Local Truly Ephemeral and Wipe-Safe (2026-06-19)
