...
| 10 — Polish | **Partly done** | `file_picker` pinned exactly rather than by caret (the caret is what allowed the drift MED-7 recorded); `googleapis_auth` 1→2 with 451 backend tests green; unused foreground-service permissions removed; governance gate grown 6 → 19 controls and able to assert a withdrawn claim stays withdrawn. **Open:** localization is the large one — 381 `Text()` literals against a catalog of one string. Four major dependency bumps are deferred *with reasons*, not forgotten |
...
| **MED-2** ◐ | Client has no state management, router, design system, localization, or responsive layout | Medium | CONFIRMED · **router, single shell, design tokens done**; state management partial (182 `setState`), localization and responsive layout open |
