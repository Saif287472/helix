...
| Remote direct-message UI data model | Component-only | `apps/helix_remote/lib/main.dart`; `apps/helix_remote/lib/screens/conversation_list_screen.dart`; `apps/helix_remote/lib/screens/conversation_screen.dart`; `apps/helix_remote/test/phase12_remote_messaging_screen_test.dart` | Phase 12 repairs the direct-message path so account setup, contact requests, conversations, messages, search, receipts, typing, edits, reactions, deletes, and blocking route through Remote services and persistent storage. Broader Remote feature screens remain later-phase work. |
| Remote composition root | Component-only | `apps/helix_remote/lib/app/composition_root.dart`; `apps/helix_remote/test` | Wires secure storage, encrypted DB, concrete REST, sync gateway, WebSocket, messaging, attachments, calls, groups, and runtime coordination. Production hardening, real-device E2E, and external security review remain release gates. |
...
| Remote crypto sessions and E2EE messaging | Defective | `packages/remote/helix_remote_crypto`; `apps/helix_remote/lib/app/remote_messaging_service.dart`; crypto and Remote messaging tests | P2-02/P2-05/P2-07/P2-09 repaired key records, prekeys, persisted local session state, X3DH transcript binding, AAD, and fail-closed sends. Full DH Double Ratchet semantics remain unproven and block strong E2EE/forward-secrecy claims. |
